...
public abstract class GT_MetaTileEntity_BasicGenerator extends GT_MetaTileEntity_BasicTank {
    ...
    public int getFuelValue(FluidStack aLiquid) {
    	...
    	FluidStack tLiquid;
    	for (GT_Recipe tFuel : getRecipes()) if ((tLiquid = GT_Utility.getFluidForFilledItem(tFuel.mInputs[0])) != null) if (aLiquid.isFluidEqual(tLiquid)) return (int)(((long)tFuel.mStartEU * getEfficiency()) / 100);
    	return 0;
    }
    
    public int getFuelValue(ItemStack aStack) {
    	if (GT_Utility.isStackInvalid(aStack)) return 0;
    	GT_Recipe tFuel = GT_Recipe.findEqualRecipe(true, getRecipes(), aStack);
    ...
    public ItemStack getEmptyContainer(ItemStack aStack) {
    	...
    	GT_Recipe tFuel = GT_Recipe.findEqualRecipe(true, getRecipes(), aStack);
    	if (tFuel != null) return tFuel.mOutputs[0];
