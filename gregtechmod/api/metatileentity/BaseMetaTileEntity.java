...
public class BaseMetaTileEntity extends BaseTileEntity implements IGregTechTileEntity {
    ...
    public void doExplosion(int aAmount) {
    	...
    	GT_Utility.sendSoundToPlayers(getWorld(), GregTech_API.sSoundList.get(209), 1.0F, -1, tX, tY, tZ);
	...
	public boolean onRightclick(EntityPlayer aPlayer, byte aSide, float aX, float aY, float aZ) {
		...
		if (isServerSide()) {
			if (!unbreakable() || aPlayer.username.equalsIgnoreCase(getOwnerName())) {
				if (getColorization() >= 0 && GT_Utility.areUnificationsEqual(new ItemStack(Item.bucketWater, 1), aPlayer.inventory.getCurrentItem())) {
				...
				if (isUpgradable() && aPlayer.inventory.getCurrentItem() != null) {
					if (GT_Utility.areUnificationsEqual(aPlayer.inventory.getCurrentItem(), GregTech_API.getGregTechComponent(25, 1))) {
					...
					if (GT_Utility.areUnificationsEqual(aPlayer.inventory.getCurrentItem(), GregTech_API.getGregTechComponent(80, 1))) {
					...
					if (GT_Utility.areUnificationsEqual(aPlayer.inventory.getCurrentItem(), GregTech_API.getGregTechComponent(89, 1))) {
					...
					if (GT_Utility.areUnificationsEqual(aPlayer.inventory.getCurrentItem(), GregTech_API.getGregTechComponent(88, 1))) {
					...
					if (getInputVoltage() < GregTech_API.VOLTAGE_HIGH && getOutputVoltage()*getOutputAmperage() < GregTech_API.VOLTAGE_HIGH){
						...
						if (GT_Utility.areUnificationsEqual(aPlayer.inventory.getCurrentItem(), GregTech_API.getGregTechComponent(27, 1))) {
							if (addTransformerUpgrade()) {
								GT_Utility.sendSoundToPlayers(getWorld(), GregTech_API.sSoundList.get(3), 1.0F, -1, getXCoord(), getYCoord(), getZCoord());
								if (!aPlayer.capabilities.isCreativeMode) aPlayer.inventory.getCurrentItem().stackSize--;
							}
					...
					if (GT_Utility.areUnificationsEqual(aPlayer.inventory.getCurrentItem(), GregTech_API.getGregTechComponent(26, 1))) {
					...
					if (GT_Utility.areUnificationsEqual(aPlayer.inventory.getCurrentItem(), GregTech_API.getGregTechComponent(12, 1))) {
					...
					if (GT_Utility.areUnificationsEqual(aPlayer.inventory.getCurrentItem(), GregTech_API.getGregTechComponent(13, 1))) {
					...
					if (GT_Utility.areUnificationsEqual(aPlayer.inventory.getCurrentItem(), GregTech_API.getGregTechComponent(14, 1))) {
					...
					if (hasMJConverterUpgrade() && GT_Utility.areUnificationsEqual(aPlayer.inventory.getCurrentItem(), GregTech_API.getGregTechComponent(28, 1))) {
						mRSEnergyCells++;
					...
					if (hasSteamEngineUpgrade() && GT_Utility.areUnificationsEqual(aPlayer.inventory.getCurrentItem(), GregTech_API.getGregTechComponent(81, 1))) {
						mSteamTanks++;
						mNeedsBatteryUpgrade = false;
						GT_Utility.sendSoundToPlayers(getWorld(), GregTech_API.sSoundList.get(3), 1.0F, -1, getXCoord(), getYCoord(), getZCoord());
						if (!aPlayer.capabilities.isCreativeMode) aPlayer.inventory.getCurrentItem().stackSize--;
	...
	public boolean addStackToSlot(int aIndex, ItemStack aStack) {
		...
		if (GT_Utility.areUnificationsEqual(getStackInSlot(aIndex), aStack) && getStackInSlot(aIndex).stackSize + aStack.stackSize <= Math.min(aStack.getMaxStackSize(), getInventoryStackLimit())) {
