...
public class GT_ModHandler {
	...
	public static boolean addScrapboxDrop(float aChance, ItemStack aOutput) {
		...
		aOutput.stackSize = 1;
		if (GT_Config.system && !GT_Utility.areUnificationsEqual(aOutput, new ItemStack(Item.hoeWood, 1, 0))) return false;
    ...
    public static boolean removeRecipeByOutput(ItemStack aOutput) {
		...
		ArrayList<IRecipe> tList = (ArrayList<IRecipe>)CraftingManager.getInstance().getRecipeList();
		for (int i = 0; i < tList.size(); i++) {
			if (GT_Utility.areUnificationsEqual(tList.get(i).getRecipeOutput(), aOutput)) {
				tList.remove(i--);
				rReturn = true;
			}
		}
    ...
    public static ItemStack getSmeltingOutput(ItemStack aInput, boolean aRemoveInput, ItemStack aOutputSlot) {
    	...
    	if (rStack != null && (aOutputSlot == null || (GT_Utility.areUnificationsEqual(rStack, aOutputSlot) && rStack.stackSize + aOutputSlot.stackSize <= aOutputSlot.getMaxStackSize()))) {
				...
				if (tEntry.getKey().matches(aInput)) {
					if (tEntry.getKey().getAmount() <= aInput.stackSize) {
						...
						for (byte i = 0; i < aOutputSlots.length; i++) {
							if (tList[i] != null) {
								if (aOutputSlots[i] == null || (GT_Utility.areUnificationsEqual(tList[i], aOutputSlots[i]) && tList[i].stackSize + aOutputSlots[i].stackSize <= aOutputSlots[i].getMaxStackSize())) {
	...
	public static int getCapsuleCellContainerCount(ItemStack aStack) {
		...
		if (GT_Utility.areUnificationsEqual(GT_Utility.getContainerForFilledItem(aStack), GT_Items.Cell_Empty.get(1)) || OrePrefixes.cell.contains(aStack) || OrePrefixes.cellPlasma.contains(aStack)) return 1;
		if (GT_Utility.areUnificationsEqual(aStack, getIC2Item("hydratedCoalCell", 1, GregTech_API.ITEM_WILDCARD_DAMAGE))) return 1;
		return 0;
	}
