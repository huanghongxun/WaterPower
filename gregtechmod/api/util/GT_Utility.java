...
public class GT_Utility {
	...
	public static byte moveStackFromSlotAToSlotB(IInventory aTileEntity1, IInventory aTileEntity2, int aGrabFrom, int aPutTo, byte aMaxTargetStackSize, byte aMinTargetStackSize, byte aMaxMoveAtOnce, byte aMinMoveAtOnce) {
		...
		if (tStack1 != null) {
			if (tStack2 != null && !areUnificationsEqual(tStack1, tStack2)) return 0;
	...
	public static boolean listContainsItem(Collection<ItemStack> aList, ItemStack aStack, boolean aTrueIfListEmpty, boolean aInvertFilter) {
		...
		while (tIterator.hasNext()) if ((tStack = tIterator.next())!= null && areUnificationsEqual(aStack, tStack)) return !aInvertFilter;
	...
	public static boolean areUnificationsEqual(ItemStack aStack1, ItemStack aStack2) {
		return areStacksEqual(aStack1, aStack2, false) || areStacksEqual(GT_OreDictUnificator.get(aStack1), GT_OreDictUnificator.get(aStack2), false);
	}
	
	public static boolean areUnificationsEqual(ItemStack aStack1, ItemStack aStack2, boolean aIgnoreNBT) {
		return areStacksEqual(aStack1, aStack2, aIgnoreNBT) || areStacksEqual(GT_OreDictUnificator.get(aStack1), GT_OreDictUnificator.get(aStack2), aIgnoreNBT);
	}
	...
	public static ItemStack getContainerForFilledItem(ItemStack aStack) {
		...
		for (FluidContainerData tData : FluidContainerRegistry.getRegisteredFluidContainerData())
			if (areUnificationsEqual(tData.filledContainer, aStack))
	...
	public static boolean removeSimpleIC2MachineRecipe(ItemStack aInput, Map<IRecipeInput, RecipeOutput> aRecipeList, ItemStack aOutput) {
		...
		for (Map.Entry<IRecipeInput, RecipeOutput> tEntry : aRecipeList.entrySet()) {
			if (aInput == null || tEntry.getKey().matches(aInput)) {
				...
				if (tList != null) for (ItemStack tOutput : tList) if (aOutput == null || areUnificationsEqual(tOutput, aOutput)) {
    ...
    public static void scanPipes(GT_MetaPipeEntity_Item aMetaTileEntity, Map<GT_MetaPipeEntity_Item, Long> aMap, long aStep, boolean aSuckItems) {
