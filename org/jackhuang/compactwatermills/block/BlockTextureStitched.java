...
public class BlockTextureStitched extends TextureAtlasSprite
{
  ...
  public boolean loadSubImage(Resource res) throws IOException {
    ...
    if (cacheEntry != null) {
      ...
      cachedImages.put(name, new BlockTextureStitched.CacheEntry(bufferedImage, animationMeta));
    }
...
private boolean loadFrames(BufferedImage image, int index, int animationLength)
  {
    ...
    List<BlockTextureStitched> matchingTextures = existingTextures.get(Integer.valueOf(hash));
    ...
    if (matchingTextures != null) {
      ...
      for (BlockTextureStitched matchingTexture : matchingTextures) {
        if ((matchingTexture.comparisonImage.getWidth() != this.width) || (matchingTexture.comparisonImage.getHeight() != totalHeight)) {
          continue;
        }
    ...
    if ((this.animationMeta != null) && (this.animationMeta.getFrameCount() > 0))
      for (it = this.animationMeta.getFrameIndexSet().iterator(); it.hasNext(); ) {
        Integer frameIndex = (Integer)it.next();
        if (frameIndex.intValue() >= animationLength) throw new RuntimeException("invalid frame index: " + frameIndex + " (" + getIconName() + ")");

        while (this.framesTextureData.size() <= frameIndex.intValue()) this.framesTextureData.add(null);

        this.framesTextureData.set(frameIndex.intValue(), Arrays.copyOfRange(rgbaData, frameIndex.intValue() * pixelsPerFrame, (frameIndex.intValue() + 1) * pixelsPerFrame));
      }
    else {
      for (int i = 0; i < animationLength; i++) {
        this.framesTextureData.add(Arrays.copyOfRange(rgbaData, i * pixelsPerFrame, (i + 1) * pixelsPerFrame));
      }
    }
