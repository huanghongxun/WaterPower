...
import org.jackhuang.compactwatermills.common.item.range.RangeInventorySlot;
...
public class TileEntityWatermill extends TileEntityElectricMetaBlock {
	...
	private void getWaterBlocks() {
		...
		for (int xTest = -range; xTest <= range; xTest++) {
			for (int yTest = -range; yTest <= range; yTest++) {
				for (int zTest = -range; zTest <= range; zTest++) {
					if (Utilities.isWater(worldObj, xCoord + xTest, yCoord + yTest, zCoord + zTest)) {
						waterBlocks++;
					} else if(Utilities.isLava(worldObj, xCoord + xTest, yCoord + yTest, zCoord + zTest)) {
						lavaBlocks++;
					}
				}
			}
		...
		if(type.ordinal() < 2) lavaBlocks = 0;
	}
