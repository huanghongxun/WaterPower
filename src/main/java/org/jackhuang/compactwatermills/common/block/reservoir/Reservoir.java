...
//				&& world.getBlockMetadata(x, y, z) == type;
	...
	//  W   | O -- begin to test(x, y, z)
	//  O
	public static Pair getVerticalWallWater(World world,
			int x, int y, int z,
			int width, int height, int meta) {
		int i, j, k = 0, l = 0;
		for (i = z; i <= z + width - 1; i++)
			for (j = y; j <= y + height - 1; j++)
				if(Utilities.isWater(world, x, j, i))
	...
	// OWWW  | T -- turbine
	//       | O -- begin to test(x, y, z)
	public static Pair getHorizontalWallWater(World world,
			int x, int y, int z,
			int width, int height, int meta) {
		int i, j, k = 0, l = 0;
		for (i = x; i <= x + width - 1; i++)
			for (j = y; j <= y + height - 1; j++)
				if(Utilities.isWater(world, i, j, z))
	...
	// WWW
	// OWW
	public static Pair getFloorWater(World world,
			int x, int y, int z,
			int length, int width, int meta) {
		int i, j, k = 0, l = 0;
		for (i = x; i <= x + length - 1; i++)
			for (j = z; j <= z + width - 1; j++)
				if(Utilities.isWater(world, i, y, j))
