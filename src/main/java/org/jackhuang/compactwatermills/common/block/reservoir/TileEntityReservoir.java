...
import org.jackhuang.compactwatermills.common.block.turbines.Position;
import org.jackhuang.compactwatermills.common.block.watermills.WaterType;
import org.jackhuang.compactwatermills.common.inventory.InventorySlotConsumableLiquid;
import org.jackhuang.compactwatermills.common.inventory.InventorySlotConsumableLiquidByList;
import org.jackhuang.compactwatermills.common.inventory.InventorySlotOutput;
import org.jackhuang.compactwatermills.common.inventory.InventorySlotUpgrade;
import org.jackhuang.compactwatermills.common.inventory.remote.InventorySlotOutputRemote;
import org.jackhuang.compactwatermills.common.inventory.remote.InventorySlotUpgradeRemote;
import org.jackhuang.compactwatermills.common.network.CompactWatermillsPacket;
import org.jackhuang.compactwatermills.common.tileentity.TileEntityMetaMultiBlock;
...
import org.jackhuang.compactwatermills.util.Pair;
...
public class TileEntityReservoir extends TileEntityMetaMultiBlock implements
	...
	@Override
	public CompactWatermillsPacket serverSendMultiBlocks() {
		return new CompactWatermillsPacket(1, null, new int[] { xCoord, yCoord,
	...
	protected void onUpdate() {
		...
		double[] ds = Utilities.getBiomeRaining(worldObj, xCoord, zCoord);
		...
		//雨水接收
		addWater += rainLevel * (underLevel / 4) * (overLevel / 4) * weather * add;//(int) ((area - cover) * 2 * weather * biomeGet);
		//潮汐接收
		addWater += Math.min(this.ocean.k, this.ocean.t) * type.capacity / 100;
		//地底接收
		addWater += underLevel * (overLevel / 4) * (rainLevel / 4) * (1D - this.yCoord / 256) * add;
		//地表接收
		addWater += overLevel * (underLevel / 4) * (rainLevel / 4) * (1D - Math.abs(64 - this.yCoord) / 64) * biomeGet * add;
		...
		highPotentialEnergyWater += addWater * CompactWatermills.updateTick;
	...
	public void refreshPlugins() {
		...
		for (int i = 0; i < this.upgradeSlot.size(); i++) {
			...
			if ((stack == null) || (!(stack.getItem() instanceof IPluginHandler)))
					continue;
			IPluginHandler upgrade = (IPluginHandler) stack.getItem();
