...
public class MachineRecipes extends CraftGuideAPIObject implements RecipeProvider {
    ...
    public void generateRecipes(RecipeGenerator generator) {
        addMachineRecipes(generator, GlobalBlocks.lathe, Arrays.asList(GlobalBlocks.lathe), MyRecipes.lathe);
        addMachineRecipes(generator, GlobalBlocks.sawmill, Arrays.asList(GlobalBlocks.sawmill), MyRecipes.sawmill);
        addMachineRecipes(generator, GlobalBlocks.compressor, Arrays.asList(GlobalBlocks.compressor), MyRecipes.compressor);
        addMachineRecipes(generator, GlobalBlocks.macerator, Arrays.asList(GlobalBlocks.macerator), MyRecipes.macerator);
    }
    ...
    private void addMachineRecipes(RecipeGenerator generator, ItemStack type, Object machine, IRecipeManager recipeManager) {
        ...
        recipeSlots[2] = new EUSlot(columns > 1 ? 23 : 31, 12).setConstantPacketSize(2).setConstantEUValue(-800);
