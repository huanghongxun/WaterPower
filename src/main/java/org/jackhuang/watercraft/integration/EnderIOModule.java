...
public class EnderIOModule {
    public static void alloySmelter(String name, ItemStack o, IMyRecipeInput... i) {
        ...
        String value = String.format("<recipeGroup name=\"WaterPower\">" + "<recipe name=\"%s\" energyCost=\"10000\">" + "<input>" + list + "</input>" + "<output>" + "<itemStack modID=\"%s\" itemName=\"%s\" itemMeta=\"%s\" exp=\"1\" number=\"%d\" />" + "</output>" + "</recipe>" + "</recipeGroup>", name, Reference.ModID, o.getItem().delegate.name().split(":")[1], o.getItemDamage(), o.stackSize);
