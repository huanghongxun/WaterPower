...
public class BlockWatermill extends BlockRotor {
    ...
    protected int getTextureIndex(IBlockAccess iBlockAccess, int x, int y, int z, int meta) {
        ...
        if (tTileEntity instanceof TileEntityWatermill) {
            if (((TileEntityWatermill) tTileEntity).getType() == null)
                return meta;
    ...
    public void registerBlockIcons(IIconRegister iconRegister) {
        ...
        for (int i = 0; i < maxMetaData(); i++) {
            ...
            textures[i][1] = iconUp;
            textures[i][2] = textures[i][3] = textures[i][4] = textures[i][5] = iconRegister.registerIcon(Reference.ModID + ":watermill/" + getTextureName(i));
        }
    }
