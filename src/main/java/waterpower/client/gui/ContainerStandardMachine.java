...
public class ContainerStandardMachine extends ContainerFullInventory {
    ...
    public ContainerStandardMachine(EntityPlayer entityPlayer, TileEntityStandardWaterMachine tileEntity) {
        this(entityPlayer, tileEntity, 2);
    }

    public ContainerStandardMachine(EntityPlayer entityPlayer, TileEntityStandardWaterMachine tileEntity, int outputNum) {
    ...
    protected void paintLayout() {
        ...
        addSlotToContainer(new SlotInventorySlot(tileEntity.inputSlot, 0, inputX, inputY));

        for (int i = 0; i < outputNum; i++)
            addSlotToContainer(new SlotInventorySlot(tileEntity.outputSlot, i, outputX + i * 18, outputY));

        for (int i = 0; i < 4; i++)
            addSlotToContainer(new SlotInventorySlot(tileEntity.upgradeSlot, i, upgradeX, upgradeY + i * 18, 2));
    }
}