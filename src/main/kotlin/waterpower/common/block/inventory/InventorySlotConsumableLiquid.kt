...
class InventorySlotConsumableLiquid(base: TileEntityInventory, name: String, access: InventorySlot.Access, count: Int, preferredSide: InventorySlot.InvSide = InventorySlot.InvSide.TOP, private var opType: InventorySlotConsumableLiquid.OpType = OpType.Drain)
    ...
    override fun accepts(stack: ItemStack?): Boolean {
        ...
        if (this.opType == OpType.Drain || this.opType == OpType.Both) {
            ...
            if (FluidContainerRegistry.isFilledContainer(stack)) { // to support old mods
                containerFluid = FluidContainerRegistry.getFluidForFilledItem(stack)
            } else if (item is IFluidContainerItem) {
                containerFluid = item.getFluid(stack)
            } else if (stack.hasCapability(CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY, null)) {
                val singleStack = stack.copyWithNewCount(1)
                val handler = singleStack.getCapability(CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY, null)
                if (handler != null) {
        ...
        if (this.opType == OpType.Fill || this.opType == OpType.Both) {
            if (FluidContainerRegistry.isEmptyContainer(stack)) {
                if (possibleFluids == null) return true
                else for (fluid in possibleFluids!!)
                    if (FluidContainerRegistry.fillFluidContainer(FluidStack(fluid, Int.MAX_VALUE), stack) != null)
                        return true
            } else {
                if (item is IFluidContainerItem) {
                    val prevFluid = item.getFluid(stack)
                    if (prevFluid == null || item.getCapacity(stack) > prevFluid.amount) {
                        if (possibleFluids == null) return true
                        else {
                            val single = stack.copyWithNewCount(1)
                            for (fluid in possibleFluids!!)
                                if (item.fill(single, FluidStack(fluid, Int.MAX_VALUE), false) > 0)
                                    return true
                        }
                    }
                } else if (stack.hasCapability(CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY, null)) {
                    val single = stack.copyWithNewCount(1)
                    val handler = stack.getCapability(CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY, null) as net.minecraftforge.fluids.capability.IFluidHandler?
                    if (handler != null) {
                        val fluid = handler.drain(Int.MAX_VALUE, false)
                        if (fluid == null) {
                            ...
                            val copy = fluid.copy()
                            copy.amount = Int.MAX_VALUE
                            if (handler.fill(copy, false) > 0)
                                return true
                        }
                    }
                }
